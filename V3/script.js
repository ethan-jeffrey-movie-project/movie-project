...
function loadHorrorMovies(swiperId) {
	...
	&sort_by=popularity.desc&include_adult=false&include_video=false&page=1&with_genres=27,35
	...
	&with_theaters=true&with_movie_discovery=true&with_theaters.lat=${lat}&with_theaters.lng=${lng}&with_theaters.radius=100km`;
		...
		fetch(url)
			...
			.then((data) => {
				...
				const movies = data.results.filter((movie) => movie.genre_ids.includes(27) || movie.genre_ids.includes(35));
