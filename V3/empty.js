      ...
      &with_theaters=true&with_movie_discovery=true&with_theaters.lat=${lat}&with_theaters.lng=${lng}&with_theaters.radius=100km`;
		...
		Promise.all([
			...
			.then(([horrorData, actionData]) => {
				...
				const horrorMovies = horrorData.results.filter((movie) => movie.genre_ids.includes(27));
				const actionMovies = actionData.results.filter((movie) => movie.genre_ids.includes(28));

				// Populate the horror movies container
				const horrorSwiperWrapper = document.querySelector(`#${horrorSwiperId} .swiper-wrapper`);
				horrorSwiperWrapper.innerHTML = "";
				let slideIndex = -1;
				horrorMovies.forEach((movie, index) => {
					// Create a new slide every 5th movie
					if (index % 5 === 0) {
						slideIndex++;
						const slide = document.createElement("div");
						slide.classList.add("swiper-slide");
						horrorSwiperWrapper.appendChild(slide);
						slide.classList.add("swiper-slide");
						slide.classList.add("justify-content-between");
						slide.classList.add("d-flex");
					}

					const slide = horrorSwiperWrapper.children[slideIndex];
				...
				const actionSwiperWrapper = document.querySelector(`#${actionSwiperId} .swiper-wrapper`);
				actionSwiperWrapper.innerHTML = "";
				slideIndex = -1;
				actionMovies.forEach((movie, index) => {
					...
					if (index % 5 === 0) {
						...
						const slide = document.createElement("div");
						slide.classList.add("swiper-slide");
						actionSwiperWrapper.appendChild(slide);
						slide.classList.add("swiper-slide");
						slide.classList.add("justify-content-between");
						slide.classList.add("d-flex");
					}

					const slide = actionSwiperWrapper.children[slideIndex];
